...
- Mobile-first formatting: Most viewers read on phones where YouTube truncates aggressively. The FIRST LINE is critical — it must be a compact "power hook" that fits before the Show More button (~100 chars). Pack the key location, topic, and format tag into one punchy line. After that, use short paragraphs (2-3 sentences max) with blank lines between them. No walls of text.
...
Return a JSON object with exactly this shape:
{
  ...
  "descriptionEn": ${languageOutput !== "jp" ? (languageOutput === "en+jp" ? '"English section of a bilingual YouTube description. LINE 1 MUST be a compact bilingual power hook — one short English phrase + Japanese equivalent + optional emoji + format tag, ALL ON ONE LINE. This is what mobile users see before Show More. Example format: Experience Nakameguro peak sakura bloom. 満開の中目黒の桜をお楽しみください。🌸 [4K Walking Tour]. Then \\n\\n, then 2-3 short English paragraphs with keywords woven naturally. End with a --- divider. 100-150 words total."' : '"SEO description in English. LINE 1 MUST be a compact power hook — one punchy sentence with major keywords + format tag, all fitting in ~100 chars. This is what mobile users see before Show More. Then \\n\\n, then 2-3 short paragraphs describing what viewers will see, the atmosphere, and context. Mobile-first: no walls of text. NEVER use pipe-separated keyword lists. 150-250 words total."') : '""'},
